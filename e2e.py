...
def test_scores_service(url):
    try:
        ...
        full_bin_path = webdriver.Chrome(executable_path=BIN_CHROME_DRIVER_DEST)
        my_driver = webdriver.Chrome(service=ChromeService(BIN_CHROME_DRIVER_DEST), options=chrome_options)
        my_driver.get(url)
